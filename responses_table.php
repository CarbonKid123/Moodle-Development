...
class mod_feedback_responses_table extends table_sql {
    ...
    protected function init($group = 0) {
        ...
        $this->column_class['fullname'] .= ' average-heading'; //Code modified by Omkar Sawant
...
public function build_table() {
    ...
    //Code modified by Omkar Sawant to calculate the column averages
    $chunk = [];
    $averages = []; // Array to store the column averages
    foreach ($this->rawdata as $row) {
        if ($this->hasmorecolumns) {
            ...
            foreach ($row as $key => $value) {
                if (preg_match('/^val(\d+)$/', $key, $matches)) {
                    $id = $matches[1];
                    if (!isset($averages[$id])) {
                        $averages[$id] = 0;
                    }
                    $averages[$id] += (float)$value;
                }
            }
    ...
    // Calculate the column averages and add a new row for displaying them Code modified by Omkar Sawant
    $numColumns = count($this->headers);
    $averageRow = [];
    foreach ($averages as $id => $sum) {
             $average = $sum / $this->totalrows;
             $averageRow["val{$id}"] = round($average, 2); // Round to one decimal point
    }

    // Add the "Average" row to the table with the heading "Average"
    $this->add_data_keyed($averageRow, 'average-row');
    
    $averageTitleRow = [];
    for ($i = 0; $i < $numColumns; $i++) {
         $averageTitleRow[] = '';
    }
    $averageTitleRow[0] = 'Average';

    $this->add_data_keyed($averageTitleRow, 'average-row');       
}
